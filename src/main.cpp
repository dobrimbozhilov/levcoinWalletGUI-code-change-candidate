...
int main(int argc, char* argv[]) {
  ...
  QSettings levcoinKey("HKEY_CLASSES_ROOT\\levcoin", QSettings::NativeFormat);
  levcoinKey.setValue(".", "Levcoin Wallet");
  levcoinKey.setValue("URL Protocol", "");
  QSettings levcoinOpenKey("HKEY_CLASSES_ROOT\\levcoin\\shell\\open\\command", QSettings::NativeFormat);
  levcoinOpenKey.setValue(".", "\"" + QCoreApplication::applicationFilePath().replace("/", "\\") + "\" \"%1\"");
#endif
  ...
  //as root
  args << "-c" << "printf '[Desktop Entry]\\nName = Levcoin URL Handler\\nGenericName = Levcoin\\nComment = Handle URL Sheme levcoin://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/levcoin;\\nIcon = Karbo-Wallet' | tee /usr/share/applications/levcoin-handler.desktop";
