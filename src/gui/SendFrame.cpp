...
void SendFrame::parsePaymentRequest(QString _request) {
    ...
    if(_request.startsWith("levcoin://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "levcoin:");
    }
    if(!_request.startsWith("levcoin:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with levcoin:"), QtCriticalMsg));
      return;
    }
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "bg1dSSXsTPdZNsupB3hh6WWYpezCazEjXS86gFY8ZFruGyfkUqTyDPsUh9MP6Sqp3wSw9RdQtdHupKVrM5gycLxx2rxUrUfPe";
