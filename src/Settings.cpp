...
void Settings::load() {
  ...
  defaultPoolList << "pool.mydevstand.com:3333" << "pool.levcoin.net:3333" << "levcoin.ourserver.net:3333" << "lvc.mining.garden:3333";
  ...
  defaultNodesList << "pool.mydevstand.com:21725"; 
...
QString Settings::getConnection() const {
    if (m_settings.contains(OPTION_CONNECTION)) {
		return m_settings.value(OPTION_CONNECTION).toString();
    }
    else {
		return "auto"; // default
    }
